...
endpoint = '/7/sync/policies'
response = session.get(papi + endpoint, verify=False)
result = json.loads(response.content)
for policy in result['policies']:
    print("Policy Name: %s -- Action: %s -- Source Path: %s -- Target Host %s -- Target Path: %s"
          % (policy['name'], policy['action'], policy['source_root_path'], policy['target_host'],
             policy['target_path']))
